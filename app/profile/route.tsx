...
export async function GET(request: Request) {
  ...
  const username = searchParams.get("username") ?? "Đang cập nhật";
      ...
      >
        <div tw="flex w-full h-full flex-col">
          {banner && (
            <img
              src={banner}
              tw="w-full h-1/2 flex-1"
              style={{ objectFit: "cover" }}
            />
          )}
          <div tw="flex w-full p-6 -mt-36 items-end flex-shrink-0 justify-between">
            <div tw="flex items-end">
              {avatar && (
                <img
                  src={avatar}
                  tw="w-72 h-72 border-8 border-zinc-900 rounded-full"
                />
              )}
              <div tw="flex flex-col p-8">
                <span tw="mb-3" style={{ fontSize: "3rem" }}>
                  {name}
                </span>
                <span tw="text-zinc-500">{username}</span>
              </div>
            </div>

            <div tw="flex mb-6 mr-6">
              <svg
